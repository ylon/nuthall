...
class Yggtorrent {
  ...
  search(query, category, limit) {
    ...
    return this.torrentSearch.search(query, category, limit)
  ...
  last(category, limit) {
    limit = limit ? parseInt(limit) : null
    return this.torrentSearch.last(category, limit)
      .then(torrents => this.format_torrents(torrents))
  }
  ...
  torrent_link_from_id(id) {
    return this.ygg.scrapeDatas.baseUrl + format('/engine/download_torrent?id={id}', {
      id: id
    })
  }
}
